...
CREATE TABLE `match_table` (
  ...
  `select_time` varchar(45) NOT NULL,
  `select_favor_sex` varchar(45) DEFAULT NULL,
  `select_favor_list` text,
  `pending` char(1) DEFAULT NULL,
  `userSex` varchar(10) DEFAULT NULL,
  `school_code` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`idx`),
  KEY `usersex_idx` (`userSex`),
  KEY `userID_idx` (`userID`,`userSex`),
  CONSTRAINT `useridfk` FOREIGN KEY (`userID`) REFERENCES `member` (`userID`)
